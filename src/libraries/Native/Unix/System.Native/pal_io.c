...
int32_t SystemNative_CopyFile(intptr_t sourceFd, intptr_t destinationFd)
{
    ...
    if (size != 0)
    {
        // Certain files (e.g. procfs) may return a size of 0 even though reading from them
        // will produce data.  As such, we can't use sendfile if the size is reported as 0.
