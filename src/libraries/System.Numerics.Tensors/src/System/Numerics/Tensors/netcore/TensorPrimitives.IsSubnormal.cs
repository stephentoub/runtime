...
namespace System.Numerics.Tensors
{
    public static partial class TensorPrimitives
    {
        ...
        /// <param name="x">The tensor, represented as a span.</param>
        /// <returns><see langword="true"/> if all of the values in <paramref name="x"/> are subnormal; otherwise, <see langword="false"/>.</returns>
        ...
        /// <param name="x">The tensor, represented as a span.</param>
        /// <returns><see langword="true"/> if any of the values in <paramref name="x"/> is subnormal; otherwise, <see langword="false"/>.</returns>
            ...
            where T : INumberBase<T>
        {
            ...
            public static Vector128<T> Invoke(Vector128<T> x)
            {
                ...
                return Vector128.IsSubnormal(x);
#else
                if (typeof(T) == typeof(float))
                {
                    Vector128<uint> bits = Vector128.Abs(x).AsUInt32();
                    return Vector128.LessThan(bits - Vector128<uint>.One, Vector128.Create(0x007F_FFFFu)).As<uint, T>();
                }

                if (typeof(T) == typeof(double))
                {
                    Vector128<ulong> bits = Vector128.Abs(x).AsUInt64();
                    return Vector128.LessThan(bits - Vector128<ulong>.One, Vector128.Create(0x000F_FFFF_FFFF_FFFFul)).As<ulong, T>();
                }
            ...
            {
#if NET10_0_OR_GREATER
                return Vector256.IsSubnormal(x);
#else
                if (typeof(T) == typeof(float))
                {
                    Vector256<uint> bits = Vector256.Abs(x).AsUInt32();
                    return Vector256.LessThan(bits - Vector256<uint>.One, Vector256.Create(0x007F_FFFFu)).As<uint, T>();
                }

                if (typeof(T) == typeof(double))
                {
                    Vector256<ulong> bits = Vector256.Abs(x).AsUInt64();
                    return Vector256.LessThan(bits - Vector256<ulong>.One, Vector256.Create(0x000F_FFFF_FFFF_FFFFul)).As<ulong, T>();
                }
            ...
            {
#if NET10_0_OR_GREATER
                return Vector512.IsSubnormal(x);
#else
                if (typeof(T) == typeof(float))
                {
                    Vector512<uint> bits = Vector512.Abs(x).AsUInt32();
                    return Vector512.LessThan(bits - Vector512<uint>.One, Vector512.Create(0x007F_FFFFu)).As<uint, T>();
                }

                if (typeof(T) == typeof(double))
                {
                    Vector512<ulong> bits = Vector512.Abs(x).AsUInt64();
                    return Vector512.LessThan(bits - Vector512<ulong>.One, Vector512.Create(0x000F_FFFF_FFFF_FFFFul)).As<ulong, T>();
                }
