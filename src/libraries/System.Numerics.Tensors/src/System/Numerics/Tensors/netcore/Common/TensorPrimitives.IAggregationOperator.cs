...
namespace System.Numerics.Tensors
{
    public static unsafe partial class TensorPrimitives
    {
        ...
        /// </typeparam>
        private static T Aggregate<T, TTransformOperator, TAggregationOperator>(
            ReadOnlySpan<T> x, ref TTransformOperator transform)
            ...
            where TAggregationOperator : struct, IAggregationOperator<T>
        {
            ...
            if (Vector512.IsHardwareAccelerated && Vector512<T>.IsSupported && TTransformOperator.Vectorizable)
            {
                ...
                if (remainder >= (uint)Vector512<T>.Count)
                {
                    result = Vectorized512(ref xRef, remainder, ref transform);
                }
                else
                {
                    // We have less than a vector and so we can only handle this as scalar. To do this
                    ...
                    result = VectorizedSmall(ref xRef, remainder, ref transform);
                }
            ...
            if (Vector256.IsHardwareAccelerated && Vector256<T>.IsSupported && TTransformOperator.Vectorizable)
            {
                ...
                if (remainder >= (uint)Vector256<T>.Count)
                {
                    result = Vectorized256(ref xRef, remainder, ref transform);
                }
                else
                {
                    // We have less than a vector and so we can only handle this as scalar. To do this
            ...
            if (Vector128.IsHardwareAccelerated && Vector128<T>.IsSupported && TTransformOperator.Vectorizable)
            {
                ...
                if (remainder >= (uint)Vector128<T>.Count)
                {
                    result = Vectorized128(ref xRef, remainder, ref transform);
                }
                else
                {
                    // We have less than a vector and so we can only handle this as scalar. To do this
            ...
            return SoftwareFallback(ref xRef, remainder, ref transform);

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T SoftwareFallback(ref T xRef, nuint length, ref TTransformOperator transform)
            {
                T result = TAggregationOperator.IdentityValue;
            ...
            static T Vectorized128(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Vector128<T> vresult = Vector128.Create(TAggregationOperator.IdentityValue);
            ...
            static T Vectorized256(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Vector256<T> vresult = Vector256.Create(TAggregationOperator.IdentityValue);
            ...
            static T Vectorized512(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Vector512<T> vresult = Vector512.Create(TAggregationOperator.IdentityValue);
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                if (sizeof(T) == 1)
                {
                    return VectorizedSmall1(ref xRef, remainder, ref transform);
                }
                else if (sizeof(T) == 2)
                {
                    return VectorizedSmall2(ref xRef, remainder, ref transform);
                }
                else if (sizeof(T) == 4)
                {
                    return VectorizedSmall4(ref xRef, remainder, ref transform);
                }
                else
                {
                    Debug.Assert(sizeof(T) == 8);
                    return VectorizedSmall8(ref xRef, remainder, ref transform);
                }
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall1(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Debug.Assert(sizeof(T) == 1);
                T result = TAggregationOperator.IdentityValue;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall2(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Debug.Assert(sizeof(T) == 2);
                T result = TAggregationOperator.IdentityValue;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall4(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Debug.Assert(sizeof(T) == 4);
                T result = TAggregationOperator.IdentityValue;
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            static T VectorizedSmall8(ref T xRef, nuint remainder, ref TTransformOperator transform)
            {
                Debug.Assert(sizeof(T) == 8);
                T result = TAggregationOperator.IdentityValue;
