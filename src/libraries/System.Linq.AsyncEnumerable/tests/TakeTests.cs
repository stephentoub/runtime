...
namespace System.Linq.Tests
{
    public class TakeTests : AsyncEnumerableTests
    {
        ...
        {
            foreach (var source in CreateSources(ints))
            {
...
#if NET
                    await AssertEqual(
                        ints.Take(new Range(new Index(1), new Index(3))),
                        source.Take(new Range(new Index(1), new Index(3))));

                    await AssertEqual(
                        ints.Take(new Range(new Index(3, fromEnd: true), new Index(1, fromEnd: true))),
                        source.Take(new Range(new Index(3, fromEnd: true), new Index(1, fromEnd: true))));
#endif
                }
            }
        }
