...
namespace System
{
    ...
    public static partial class MemoryExtensions
    {
        ...
        public static bool TryWrite(this Span<char> destination, [InterpolatedBuilderArgument("destination")] InterpolatedSpanBuilder builder, out int charsWritten)
        {
            // The span argument isn't used directly in the method; rather, it'll be used by the compiler to create the builder.
            // We could validate here that span == builder._destination, but that doesn't seem necessary.
            if (builder._success)
            {
                charsWritten = builder._pos;
                return true;
            }
        ...
        /// <param name="charsWritten">The number of characters written to the span.</param>
        /// <returns>true if the entire interpolated string could be formatted successfully; otherwise, false.</returns>
        public static bool TryWrite(this Span<char> destination, IFormatProvider? provider, [InterpolatedBuilderArgument("destination", "provider")] InterpolatedSpanBuilder builder, out int charsWritten) =>
            // The provider is passed to the builder by the compiler, so the actual implementation of the method
            // is the same as the non-provider overload.
            TryWrite(destination, builder, out charsWritten);

        /// <summary>Provides a builder used by the language compiler to format interpolated strings into character spans.</summary>
        public ref struct InterpolatedSpanBuilder
        {
            ...
            /// <param name="destination">The user-provided destination buffer.</param>
            /// <param name="success">true if the destination may be long enough; otherwise, false.</param>
            private InterpolatedSpanBuilder(Span<char> destination, bool success)
            {
            ...
            /// <param name="provider">An object that supplies culture-specific formatting information.</param>
            /// <param name="success">true if the destination may be long enough; otherwise, false.</param>
            private InterpolatedSpanBuilder(Span<char> destination, IFormatProvider? provider, bool success)
            {
                _destination = destination;
                _provider = provider;
            ...
            /// <summary>Creates a builder used to write an interpolated string into a <see cref="Span{Char}"/>.</summary>
            ...
            public static InterpolatedSpanBuilder Create(int baseLength, int holeCount, Span<char> destination, out bool success)
            {
                success = destination.Length >= baseLength;
                return new InterpolatedSpanBuilder(destination, success);
            }
            ...
            /// <param name="success">Upon return, true if the destination may be long enough to support the formatting, or false if it won't be.</param>
            /// <remarks>This is intended to be called only by compiler-generated code. Arguments are not validated as they'd otherwise be for members intended to be used directly.</remarks>
            public static InterpolatedSpanBuilder Create(int baseLength, int holeCount, Span<char> destination, IFormatProvider? provider, out bool success)
            {
                success = destination.Length >= baseLength;
                return new InterpolatedSpanBuilder(destination, provider, success);
            }
            ...
            public bool TryFormatBaseString(string value)
            {
            ...
            // Design note:
            // This provides the same set of overloads and semantics as InterpolatedStringBuilder.

            #region TryFormatInterpolationHole T
            /// <summary>Writes the specified value to the builder.</summary>
            /// <param name="value">The value to write.</param>
            public bool TryFormatInterpolationHole<T>(T value)
            {
                ...
                {
                    return TryFormatCustomFormatter(value, format: null);
                }
                ...
                if (s is not null)
                {
                    return TryFormatBaseString(s); // use TryFormatBaseString to avoid going back through this method recursively
                }
            ...
            public bool TryFormatInterpolationHole<T>(T value, string? format)
            {
                // If there's a custom formatter, always use it.
                if (_customFormatter is not null)
                {
                    return TryFormatCustomFormatter(value, format);
                }
            ...
            public bool TryFormatInterpolationHole<T>(T value, int alignment)
            {
                int startingPos = _pos;
                if (TryFormatInterpolationHole(value))
                {
            ...
            /// <param name="format">The format string.</param>
            /// <param name="alignment">Minimum number of characters that should be written for this value.  If the value is negative, it indicates left-aligned and the required minimum is the absolute value.</param>
            public bool TryFormatInterpolationHole<T>(T value, int alignment, string? format)
            {
                int startingPos = _pos;
                if (TryFormatInterpolationHole(value, format))
                {
                    return alignment == 0 || TryAppendOrInsertAlignmentIfNeeded(startingPos, alignment);
                }
            ...
            public bool TryFormatInterpolationHole(ReadOnlySpan<char> value)
            {
            ...
            /// <param name="alignment">Minimum number of characters that should be written for this value.  If the value is negative, it indicates left-aligned and the required minimum is the absolute value.</param>
            /// <param name="format">The format string.</param>
            public bool TryFormatInterpolationHole(ReadOnlySpan<char> value, int alignment = 0, string? format = null)
            {
                ...
                if (paddingRequired <= 0)
                {
                    ...
                    return TryFormatInterpolationHole(value);
                }
            ...
            /// <summary>Writes the specified value to the builder.</summary>
            /// <param name="value">The value to write.</param>
            public bool TryFormatInterpolationHole(string? value)
            {
                ...
                return TryFormatInterpolationHoleSlow(value);
            }
            ...
            private bool TryFormatInterpolationHoleSlow(string? value)
            {
                if (_customFormatter is not null)
                {
                    return TryFormatCustomFormatter(value, format: null);
            ...
            /// <param name="alignment">Minimum number of characters that should be written for this value.  If the value is negative, it indicates left-aligned and the required minimum is the absolute value.</param>
            /// <param name="format">The format string.</param>
            public bool TryFormatInterpolationHole(string? value, int alignment = 0, string? format = null) =>
            ...
            /// <param name="alignment">Minimum number of characters that should be written for this value.  If the value is negative, it indicates left-aligned and the required minimum is the absolute value.</param>
            /// <param name="format">The format string.</param>
            public bool TryFormatInterpolationHole(object? value, int alignment = 0, string? format = null) =>
            ...
            /// <param name="format">The format string.</param>
            [MethodImpl(MethodImplOptions.NoInlining)]
            private bool TryFormatCustomFormatter<T>(T value, string? format)
            {
                ...
                {
                    return TryFormatBaseString(customFormatted); // use TryFormatBaseString to avoid going back through this method recursively
                }
